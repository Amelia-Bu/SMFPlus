...
from DISTS_util import DISTS

def clear_line():
    """Clears line from any characters."""

    # print('\r{}'.format(' ' * 80), end='\r')
    logger.info('\r{}'.format(' ' * 80))
...
def progress_bar(batch_idx, num_batches, report_interval, train_loss):
    ...
    logger.info('Batch {:>{dec}d} [{}{}] Train loss: {:>1.5f}'.format(batch_idx + 1, '=' * fill + '>', ' ' * (bar_size - fill), train_loss, dec=str(dec)))
...
def show_on_epoch_end(epoch_time, valid_time, valid_loss, valid_psnr):
    ...
    logger.info('Train time: {} | Valid time: {} | Valid loss: {:>1.5f} | Avg PSNR: {:.2f} dB'.format(epoch_time, valid_time, valid_loss, valid_psnr))
...
def show_on_report(batch_idx, num_batches, loss, elapsed):
    ...
    logger.info('Batch {:>{dec}d} / {:d} | Avg loss: {:>1.5f} | Avg train time / batch: {:d} ms'.format(batch_idx + 1, num_batches, loss, int(elapsed), dec=dec))
...
def to_ssim_skimage(enhance, gt):
    ...
    ssim_list = [compare_ssim(enhance_list_np[ind],  gt_list_np[ind], data_range=1, channel_axis=-1) for ind in range(len(enhance_list))]
...
def save_image(enhance, image_name, category):
    ...
    print('[utils 222] batch_num:{}'.format(batch_num))
    File_Path = '../results/{}_results'.format(category)
    print('[utils 224] File_Path:{}'.format(File_Path))
    if not os.path.exists(File_Path):
        os.makedirs(File_Path) 
    for ind in range(batch_num):
        print('[utils 228] ind:{}  image_name[ind]:{}'.format(ind, image_name[ind][-1]))
        utils.save_image(enhance_images[ind], '../results/{}_results/{}'.format(category, image_name[ind][-1]))
...
def validation_PSNR(UHD_Net_reuse, val_data_loader, device, category, save_tag=False):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        with torch.no_grad(): 
            source_list, target_list, data_lowlight, target_gt, image_name = val_data
            source_list = [data.to(device) for data in source_list]
            target_list = [data.to(device) for data in target_list]
            data_lowlight = data_lowlight.to(device)
            target_gt = target_gt.to(device)
  
            enhance = UHD_Net_reuse(source_list, data_lowlight)
 
        # enhance = enhance.cpu()
        # out_scale_1 = out_scale_1.cpu()
        # print("enhance(utils)", enhance.cpu().device)
        # print("out(utils)", out_scale_1.cpu().device)
        # --- Calculate the average PSNR --- #
        psnr_list.extend(to_psnr(enhance[3], target_gt))
        # print("psnr list:(utils)", psnr_list.device)

        # --- Calculate the average SSIM --- # 
        ssim_list.extend(to_ssim_skimage(enhance[3], target_gt))
        # print("ssim list:(utils)", ssim_list.device)

        # print("dists list1:(utils)", dists_list.device)

        # --- Calculate the average DISTS --- #
        # print('[utils]enhance.shape: {}, gt.shape:{}'.format(enhance.shape, gt.shape))
        dists_list.extend(D(target_gt.cpu(), enhance[3].cpu()))
        # print("dists list(utils)", dists_list)
        # print("dists list:(utils)", dists_list.device)

        # --- Save image --- #
        if save_tag:
            save_image(enhance[3], image_name, category)

    avr_psnr = sum(psnr_list) / len(psnr_list) 
    avr_ssim = sum(ssim_list) / len(ssim_list)
    # print("[utils]sum(dists_list): {}, len: {}".format(sum(dists_list),len(dists_list)))
    avr_dists = sum(dists_list) / len(dists_list)
    # print('[utils]avr_dists:', avr_dists)
    return avr_psnr, avr_ssim, avr_dists#,enhance

def validation_NIQE(UHD_Net_reuse, val_data_loader, device, category, save_tag=False):
    niqe_list = []
    for batch_id, val_data in enumerate(val_data_loader):
        with torch.no_grad():
            source_list, data_lowlight, image_name = val_data
            source_list = [data.to(device) for data in source_list]
            data_lowlight = data_lowlight.to(device)

            enhance = UHD_Net_reuse(source_list, data_lowlight)
        # --- Calculate the average NIQE --- #
        iqa_metric = pyiqa.create_metric('niqe', device=device)

        niqe_list.extend(iqa_metric(enhance[2]))
        # print("psnr list:(utils)", psnr_list.device)

        # --- Save image --- #
        if save_tag:
            save_image(enhance[2], image_name, category)

    avr_niqe = sum(niqe_list) / len(niqe_list)
    # print("[utils]sum(dists_list): {}, len: {}".format(sum(dists_list),len(dists_list)))
    # print('[utils]avr_dists:', avr_dists)
    return avr_niqe, min(niqe_list)  # ,enhance
