...
class DISTS(torch.nn.Module):
    def __init__(self, load_weights=True):
        ...
        if load_weights:
            ...
            self.alpha.data = weights['alpha'].cuda()
            self.beta.data = weights['beta'].cuda()

    def forward_once(self, x):
        # print("self.std(DISTS)", self.std.device)
        # print("self.mean(DISTS)", self.mean.device)
        # print("x(DISTS)", x.device)
        h = (x - self.mean) / self.std
        # print("h(DISTS)", h.type)
        h = self.stage1(h)
        h_relu1_2 = h
        h = self.stage2(h)
        h_relu2_2 = h
    ...
    def forward(self, x, y, require_grad=False, batch_average=False):
        ...
        for k in range(len(self.chns)):
            ...
            # print("dist1(DISTS)", dist1.device)
            # print("alpha[k](DISTS)", alpha[k].device)
            # print("S1(DISTS)", S1.device)
            dist1 = dist1 + (alpha[k].cpu() * S1).sum(1, keepdim=True)

            x_var = ((feats0[k] - x_mean) ** 2).mean([2, 3], keepdim=True)
            y_var = ((feats1[k] - y_mean) ** 2).mean([2, 3], keepdim=True)
            xy_cov = (feats0[k] * feats1[k]).mean([2, 3], keepdim=True) - x_mean * y_mean
            S2 = (2 * xy_cov + c2) / (x_var + y_var + c2)
            dist2 = dist2 + (beta[k].cpu() * S2).sum(1, keepdim=True)

        score = 1 - (dist1 + dist2).squeeze()
        if batch_average:
            # print("batcha_avg(DISTS_utils)", score.mean())
            return score.mean()

        else:
            # print("score(DISTS_utils)", score)
            return score
